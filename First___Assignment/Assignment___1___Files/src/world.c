...
float Endeffector_x_world = 0.0;
float Endeffector_z_world = 0.0;
...
int main() {
	...
	while (1) {
		...
		switch (retval) {
			...
			case 2: ;
				...
				if (rand_number == 0) {
					...
					sscanf(command_mx, "%f", &Endeffector_x_world);
					close(fd_mx);
					close(fd_mz); 
					update = 1;
				}
					...
					sscanf(command_mz, "%f", &Endeffector_z_world);
					close(fd_mx);
					close(fd_mz);
					update = 2;
				}
			...
			case 1: // Only one pipe
				...
				if (FD_ISSET(fd_mx, &readfds) != 0) {
					...
					//Endeffector_x_world = atof(command_mx);
					sscanf(command_mx, "%f", &Endeffector_x_world);
					close(fd_mx);
					close(fd_mz);
					update = 1;
					...
					//Endeffector_z_world = atof(command_mz);
					sscanf(command_mz, "%f", &Endeffector_z_world);
					close(fd_mx);
					close(fd_mz);
					update = 2;
		...
		if (update != 0) {
			...
			if (update == 1) {
				Endeffector_x_world += random_error*Endeffector_x_world;
			}
			else if (update == 2) {
				Endeffector_z_world += random_error*Endeffector_z_world;
			}
		...
		sprintf(real_pos,"%f,%f",Endeffector_x_world,Endeffector_z_world);
